<div class="relative">
   @if('textarea' !== $type)
        ...
        @endif
        <input type="{{ $type }}" x-ref="input-{{ $name }}" placeholder="{{ $placeholder }}" name="{{ $name }}" value="{{ old($name, $value) }}" id="{{ $name }}"

            @class(['w-full rounded-md border-0 py-1.5 px-2.5 text-sm ring-1 placeholder:text-slate-400 focus:ring-2' => true,
            'pr-8' => $formRef,
